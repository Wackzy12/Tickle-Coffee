...
class _ravioliScreenState extends State<RavioliScreen> {
  ...
  Future<void> _checkIfFavorited() async {
    if (currentUser != null) { // Check if user is logged in
      ...
      setState(() {
        isFavorited = favorites.contains('Ravioli');
      });
    }
  }
  ...
  void _toggleFavorite() async {
    if (currentUser != null) { // Check if user is logged in
      if (isFavorited) {
        await FavoritesManager().removeFavorite(currentUser!.uid, 'Ravioli');
      } else {
        await FavoritesManager().addFavorite(currentUser!.uid, 'Ravioli');
      }
      setState(() {
