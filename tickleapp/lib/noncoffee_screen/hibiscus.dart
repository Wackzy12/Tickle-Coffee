...
class _hibiscusScreenState extends State<HibiscusScreen> {
  ...
  void _toggleFavorite() async {
    if (isFavorited) {
      await FavoritesManager().removeFavorite('Hibiscus Tea');
    } else {
      await FavoritesManager().addFavorite('Hibiscus Tea');
    }
    setState(() {
                    ...
                    onPressed: () {
                      if (totalPrice > 0 &&
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Hibiscus Tea ordered: ₱$totalPrice')),
                        );
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
